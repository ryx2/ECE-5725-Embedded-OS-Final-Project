...
sArr = np.array([]) # keep track of 2 seconds at a time
...
servoList = [] 
outputs = [[27, 18, 16],[26,17,22]] # right left, top middle bottom, same for limits array
    ...
    while True:
        ...
        try:
            ...
            if len(samps) > 0:
                ...
                if len(capTimeArray) >=1:
                    capTimeArray = np.append(capTimeArray, captureTime-storeFirst)
                else:
                    storeFirst = captureTime
                    capTimeArray = np.append(capTimeArray, 0)
        except ValueError:
##            print('sound acquisition error')
            pass
...
def beatDetect():
    ...
    while True:
        ...
        if time.time()-lastCheck > 1:
            lastCheck = time.time()
            lastCap = captureTime
            bpmEstimate,beats = librosa.beat.beat_track(y=sArr, sr=sr, hop_length = 300, units='time') # tempo is bpm, beats is where the beats are in the the sounds array
            thisTime = time.time()
...
#params are a is servo, then a direction, -1 is backward, 0 is stop, 1 is forward
toggle = True
timesCalled = np.array([])

def howLong(space): #waits until 2nd next beat from rn
    lastBeatTime = lastCap + beats[-1]
    curTime = time.time()
    timeToWait = curTime - lastBeatTime 
    print('orig time to wait is ' + str(timeToWait))
    if timeToWait < 0:
        timeToWait -= space
        freq = 60/bpm # seconds / beat
        addTime1 = (int(-timeToWait/freq)+1)*freq
        addTime2 = (int(-timeToWait/freq))*freq
        addTime = addTime1 if abs(addTime1 - timeToWait) < abs(addTime1 - timeToWait) else addTime2
        timeToWait += addTime
##                print('waiting til: ' + str(timeToWait))
    timeToWait += space
##    print('time to wait is ' + str(timeToWait))
    return timeToWait

def danceFromArray(dArr, space = 2, iter = 1): #of shape n, 2, 3 and has values 0,1,2 for low middle high
    space *= 60/bpm
    for n in range(len(dArr)):
        dArr[n][0][2] = 2 - dArr[n][0][2] 
    for _ in range(iter):
        for n in range(len(dArr)):
...
def dance():
    ...
    while True:
        try:
##            print(len(beats), bpm)
            #do a move at the next beat, if we have enough info
